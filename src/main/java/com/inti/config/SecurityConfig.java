...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http // la sécurité des requetes http
				.authorizeRequests().anyRequest().authenticated() // vérifier les requetes envoyées par les utilisateurs
																	// :
																	// http://localhost:9090/utilisateurs
																	// http://localhost:9090/login
				.and().formLogin() // formulaire login fournit par le projet Spring Security
				.permitAll().and().logout().logoutUrl("/logout").permitAll().and().httpBasic().and().csrf().disable(); // méthode
																														// d'attaque
																														// informatique
																														// (cybersécurité)
